...
class Tests_Requests(BaseTestCase):
    ...
    def test_add_request_successfully(self):
        ...
        with self.client:
            ...
            response = self.add_request(token, "mac failure", "just failed", "IT")
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 201)
            self.assertEqual(data.get('message'), "Request successfully created and sent")
            res = self.add_request(self.get_token(), "mac failure", "just failed", "IT")
    ...
    def test_get_all_requests(self):
        ...
        with self.client:
            self.register_user("hadijah", "had@gmail.com", "12345")
            token = self.get_token()
            self.add_request(token, "mac failure", "just failed", "IT")
    ...
    def test_add_request_with_invalid_title(self):
        ...
        with self.client:
            self.register_user("hadijah", "had@gmail.com", "12345")
            token = self.get_token()
            response = self.add_request(token, "m", "just failed", "IT")
            data = json.loads(response.data.decode())
    ...
    def test_add_request_with_title_with_character(self):
        ...
        with self.client:
            self.register_user("hadijah", "had@gmail.com", "12345")
            token = self.get_token()
            response = self.add_request(token, "@?.", "just failed", "IT")
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
    ...
    #         token = self.get_token()
    #         self.add_request(token, "mac failure", "just failed", "IT")
    #         response = self.put_request(token, "Ac failure", "just failed", "facilities")
    #         data = json.loads(response.data.decode())
    #         self.assertEqual(response.status_code, 201)
